...
#endif
#ifdef MODBUSASCII
t_modbusASCII xdata recvFrame;
t_modbusASCII xdata sendFrame;
#endif
...
uchar AsciitoHex8(uchar *pBuf);
uint AsciitoHex16(uchar *pBuf);
uchar LRC(uchar* auchMsg,uint usDataLen);
...
#ifdef MODBUSASCII
			if (recvLen > 0 && ':' == Com_Buffer[0] && 0x0D == Com_Buffer[recvLen - 2] && 0x0A == Com_Buffer[recvLen - 1] && equip_para.address == AsciitoHex8(&Com_Buffer[1])) {
				if (LRC(&Com_Buffer[1], recvLen - 5) == AsciitoHex8(&Com_Buffer[recvLen - 4])) {
//					Senddata(Com_Buffer, recvLen);
					switch (funcode = AsciitoHex8(&Com_Buffer[3])) {
						case 0x2B:
							recvFrame.valuelen = 6;
							break;
						case 0x06:
						case 0x03:
						case 0x05:
						case 0x01:
							recvFrame.valuelen = 8;
							break;
						default:
							SendError(0x01, funcode);
							continue;
					}
					memcpy(recvFrame.value, &Com_Buffer[5], recvFrame.valuelen);
//					Senddata(&recvFrame.address, 1);
					Responsion(funcode, recvFrame.value);
				}
			}
			recvLen = 0;			
		}
...
void Responsion(uchar funcode, uchar *value)
{
	...
	switch (funcode) {
		case 0x2B:
			//check legality
			if (strstr(value, "0E0100") == NULL) return;
			//make response
			sprintf(sendFrame.address, "%02X", (uint)equip_para.address);
			sprintf(sendFrame.funcode, "%02X", (uint)funcode);
			sendFrame.valuelen = 44;
			memcpy(sendFrame.value, "0E01010000030006", 16); i+= 16;
			memcpy(sendFrame.value + i, equip_para.serialnumber, sizeof(equip_para.serialnumber)); i += sizeof(equip_para.serialnumber);
			memcpy(sendFrame.value + i, "0108", 4); i+= 4;
			memcpy(sendFrame.value + i, equip_para.model, sizeof(equip_para.model)); i += sizeof(equip_para.model);
			memcpy(sendFrame.value + i, "0206", 4); i+= 4;
			memcpy(sendFrame.value + i, equip_para.version, sizeof(equip_para.version)); i += sizeof(equip_para.version);
			SendBuff[0] = ':';
			memcpy(SendBuff + 1, &sendFrame, sendFrame.valuelen + 4); sendLen = sendFrame.valuelen + 5;
			sprintf(lrc_ret, "%02X", LRC(&SendBuff[1], sendLen - 1));
			memcpy(SendBuff + sendLen, &lrc_ret, sizeof(lrc_ret)); sendLen += 2;
			memcpy(SendBuff + sendLen, "\r\n", 2); sendLen += 2;
			Senddata(SendBuff, sendLen);
			//do something
			break;
		case 0x06:		
			reg_address = AsciitoHex16(&value[0]);
			reg_value = AsciitoHex16(&value[4]);
			if (reg_address == 0x0001) {
				if (reg_value > 0 && reg_value <= 32) {
					if (equip_para.address != reg_value) {
						...
						sprintf(sendFrame.address, "%02X", (uint)equip_para.address);
						sprintf(sendFrame.funcode, "%02X", (uint)funcode);
						sendFrame.valuelen = 8;
						sprintf(sendFrame.value, "0001");
						sprintf(sendFrame.value + 4, "%04X", (uint)reg_value);
						SendBuff[0] = ':';
						memcpy(SendBuff + 1, &sendFrame, sendFrame.valuelen + 4); sendLen = sendFrame.valuelen + 5;
						sprintf(lrc_ret, "%02X", LRC(&SendBuff[1], sendLen - 1));
						memcpy(SendBuff + sendLen, &lrc_ret, sizeof(lrc_ret)); sendLen += 2;
				...
				if (reg_value > 0 && reg_value <= equip_para.maxpressure) {
					...
					sprintf(sendFrame.funcode, "%02X", (uint)funcode);
					sendFrame.valuelen = 8;
					sprintf(sendFrame.value, "0002");
					sprintf(sendFrame.value + 4, "%04X", (uint)set_pressure);
					SendBuff[0] = ':';
					memcpy(SendBuff + 1, &sendFrame, sendFrame.valuelen + 4); sendLen = sendFrame.valuelen + 5;
					sprintf(lrc_ret, "%02X", LRC(&SendBuff[1], sendLen - 1));
					memcpy(SendBuff + sendLen, &lrc_ret, sizeof(lrc_ret)); sendLen += 2;
				...
				if (reg_value > 0 && reg_value <= equip_para.maxflow) {
					...
					sprintf(sendFrame.funcode, "%02X", (uint)funcode);
					sendFrame.valuelen = 8;
					sprintf(sendFrame.value, "0003");
					sprintf(sendFrame.value + 4, "%04X", (uint)set_flow);
					SendBuff[0] = ':';
					memcpy(SendBuff + 1, &sendFrame, sendFrame.valuelen + 4); sendLen = sendFrame.valuelen + 5;
					sprintf(lrc_ret, "%02X", LRC(&SendBuff[1], sendLen - 1));
					memcpy(SendBuff + sendLen, &lrc_ret, sizeof(lrc_ret)); sendLen += 2;
		...
		case 0x03:
			...
			if (reg_address == 0x0002) {
				if (reg_value == 0x0001) {
					...
					sprintf(sendFrame.address, "%02X", (uint)equip_para.address);
					sprintf(sendFrame.funcode, "%02X", (uint)funcode);
					sendFrame.valuelen = 6;
					sprintf(sendFrame.value, "02");
					sprintf(sendFrame.value + 2, "%04X", (uint)set_pressure);
					SendBuff[0] = ':';
					memcpy(SendBuff + 1, &sendFrame, sendFrame.valuelen + 4); sendLen = sendFrame.valuelen + 5;
					sprintf(lrc_ret, "%02X", LRC(&SendBuff[1], sendLen - 1));
					memcpy(SendBuff + sendLen, &lrc_ret, sizeof(lrc_ret)); sendLen += 2;
				...
				if (reg_value == 0x0001) {
					...
					sendFrame.valuelen = 6;
					sprintf(sendFrame.value, "02");
					sprintf(sendFrame.value + 2, "%04X", (uint)set_flow);
					SendBuff[0] = ':';
					memcpy(SendBuff + 1, &sendFrame, sendFrame.valuelen + 4); sendLen = sendFrame.valuelen + 5;
					sprintf(lrc_ret, "%02X", LRC(&SendBuff[1], sendLen - 1));
					memcpy(SendBuff + sendLen, &lrc_ret, sizeof(lrc_ret)); sendLen += 2;
				...
				if (reg_value == 0x0001) {
					...
					sendFrame.valuelen = 6;
					sprintf(sendFrame.value, "02");
					sprintf(sendFrame.value + 2, "%04X", (uint)(VAL_pnow / 10));
					SendBuff[0] = ':';
					memcpy(SendBuff + 1, &sendFrame, sendFrame.valuelen + 4); sendLen = sendFrame.valuelen + 5;
					sprintf(lrc_ret, "%02X", LRC(&SendBuff[1], sendLen - 1));
					memcpy(SendBuff + sendLen, &lrc_ret, sizeof(lrc_ret)); sendLen += 2;
		...
		case 0x05:
			...
			if (reg_address == 0x0001) {
				if (reg_value == 0xff00) {
					...
					sendFrame.valuelen = 8;
					sprintf(sendFrame.value, "0001");
					sprintf(sendFrame.value + 4, "FF00");
					SendBuff[0] = ':';
					memcpy(SendBuff + 1, &sendFrame, sendFrame.valuelen + 4); sendLen = sendFrame.valuelen + 5;
					sprintf(lrc_ret, "%02X", LRC(&SendBuff[1], sendLen - 1));
					memcpy(SendBuff + sendLen, &lrc_ret, sizeof(lrc_ret)); sendLen += 2;
					...
					sendFrame.valuelen = 8;
					sprintf(sendFrame.value, "0001");
					sprintf(sendFrame.value + 4, "0000");
					SendBuff[0] = ':';
					memcpy(SendBuff + 1, &sendFrame, sendFrame.valuelen + 4); sendLen = sendFrame.valuelen + 5;
					sprintf(lrc_ret, "%02X", LRC(&SendBuff[1], sendLen - 1));
					memcpy(SendBuff + sendLen, &lrc_ret, sizeof(lrc_ret)); sendLen += 2;
		...
		case 0x01:
			...
			if (reg_address == 0x0001) {
				if (reg_value == 0x0001) {
					...
					sprintf(sendFrame.value, "01");
					sprintf(sendFrame.value + 2, "%02X", (uint)equip_para.pumpstate);
					SendBuff[0] = ':';
					memcpy(SendBuff + 1, &sendFrame, sendFrame.valuelen + 4); sendLen = sendFrame.valuelen + 5;
					sprintf(lrc_ret, "%02X", LRC(&SendBuff[1], sendLen - 1));
					memcpy(SendBuff + sendLen, &lrc_ret, sizeof(lrc_ret)); sendLen += 2;
...
#endif
#ifdef MODBUSASCII
		Com_Buffer[recvLen++]=SBUF & 0x7F;
#endif
		if (recvLen >= sizeof(Com_Buffer)) recvLen = 0;
		TL0 = 0x00;
		TH0 = 0xCF;
		TR0 = 1;
	}
...
void SendError(uchar no, uchar funcode)
{
	...
	if (no == 0x01) {
		//make response
		sprintf(sendFrame.address, "%02X", (uint)equip_para.address);
		sprintf(sendFrame.funcode, "%02X", (uint)(funcode + 0x80));
...
{
	uchar byteH, byteL;
    if(pBuf[0] >= '0' && pBuf[0] <= '9') byteH = pBuf[0] - '0';
	else if(pBuf[0] >= 'A' && pBuf[0] <= 'F') byteH = pBuf[0] - 'A' + 10;
    if(pBuf[1] >= '0' && pBuf[1] <= '9') byteL = pBuf[1] - '0';
	else if(pBuf[1] >= 'A' && pBuf[1] <= 'F') byteL = pBuf[1] - 'A' + 10;
	
    return byteH << 4 | byteL;
}
uint AsciitoHex16(uchar *pBuf)
{
	uchar byteHH, byteH, byteL, byteLL;
    if(pBuf[0] >= '0' && pBuf[0] <= '9') byteHH = pBuf[0] - '0';
	else if(pBuf[0] >= 'A' && pBuf[0] <= 'F') byteHH = pBuf[0] - 'A' + 10;
    if(pBuf[1] >= '0' && pBuf[1] <= '9') byteH = pBuf[1] - '0';
	else if(pBuf[1] >= 'A' && pBuf[1] <= 'F') byteH = pBuf[1] - 'A' + 10;
    if(pBuf[2] >= '0' && pBuf[2] <= '9') byteL = pBuf[2] - '0';
	else if(pBuf[2] >= 'A' && pBuf[2] <= 'F') byteL = pBuf[2] - 'A' + 10;
    if(pBuf[3] >= '0' && pBuf[3] <= '9') byteLL = pBuf[3] - '0';
	else if(pBuf[3] >= 'A' && pBuf[3] <= 'F') byteLL = pBuf[3] - 'A' + 10;
	
    return byteHH << 12 | byteH << 8 | byteL << 4 | byteLL;
}
